...
static const char* bgJpegPath = "/background.jpg";
static const char* bgPngPath = "/background.png";
...
static void drawBootScreen(uint8_t progress, const char* status) {
    ...
    drawBootTextCentered("SIGNAL DECK v4.3", 114, 1, TFT_CYAN);
...
static bool parseGeoTargetLine(char* line, double* lat, double* lon,
    ...
    *lat = atof(latField);
    *lon = atof(lonField);
    uint16_t parsedRadius = static_cast<uint16_t>(atoi(radiusField));
    *radiusM = parsedRadius > 0 ? parsedRadius : geoRadiusMeters;
...
static void drawBackground() {
    ...
    if (backgroundEnabled) {
        if (SD.exists(bgJpegPath)) {
            drawn = M5.Display.drawJpgFile(SD, bgJpegPath, 0, 0);
        } else if (SD.exists(bgPngPath)) {
            drawn = M5.Display.drawPngFile(SD, bgPngPath, 0, 0);
        }
    }
    if (!drawn) {
...
static void drawScreenSaver() {
    ...
    drawBootTextCentered("SIGNAL DECK v4.3", 112, 1, TFT_CYAN);
...
static void drawMenuFrame() {
    ...
    for (int i = 0; i < visibleCount; i++) {
        ...
        if (itemIndex == 2) {
            ...
            String label = String(menuItems[itemIndex]) + ": " + String(gpsPortName());
            M5.Display.drawString(label, 36, y);
        } else if (itemIndex == 8) {
            String label = String(menuItems[itemIndex]) + (geoAlertEnabled ? ": On" : ": Off");
            M5.Display.drawString(label, 36, y);
        } else if (itemIndex == 9) {
            String label = String(menuItems[itemIndex]) + ": " + String(geoRadiusMeters) + "m";
            M5.Display.drawString(label, 36, y);
        } else if (itemIndex == 10) {
            String label = String(menuItems[itemIndex]) + (backgroundEnabled ? ": On" : ": Off");
            M5.Display.drawString(label, 36, y);
...
static bool saveSettingsToSd() {
    ...
    doc["background"] = backgroundEnabled;
...
static bool loadSettingsFromSd() {
    ...
    backgroundEnabled = doc["background"] | false;
...
static void drawAlertPopup() {
    ...
    if (alertPopupGeo) {
        ...
        String geoLine = String((int)nearestGeoDistanceM) + "m " + String(nearestGeoLabel);
