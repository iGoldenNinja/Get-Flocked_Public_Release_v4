...
static const char* debugMenuDescriptions[] = {
    "Red alert popup and RF tone.",
    "Blue GEO popup and GEO tone.",
    "Flash R/G/B/W/black panels.",
    "Play the boot tone phrase.",
    "Write manual_marker to log.",
    "Show raw GPS fix data.",
    "Check geo index/group files.",
    "Append /debug_test.txt.",
    "Show BLE scan counters.",
    "Show WiFi channel state.",
    "Return to main menu."
};
...
static bool geoFileTestHasIndex = false;
static uint16_t geoFileTestGroupRefs = 0;
static uint16_t geoFileTestGroupsFound = 0;
static uint16_t geoFileTestActiveGroups = 0;
static uint16_t geoFileTestMapTargets = 0;
static float geoFileTestNearestM = -1.0f;
static char geoFileTestNearestLabel[24] = "--";
static uint32_t lastDebugViewUpdate = 0;
...
static void playStartupChimeSoft();
...
bool SoundEngine::playDefaultToneSequence(const char* soundName, bool asyncMode) {
    ...
    if (strcmp(soundName, "startup") == 0) {
        if (asyncMode) {
            M5.Speaker.tone(880, 170);
            return true;
        }
        M5.Speaker.tone(523, 120);
        delay(155);
        M5.Speaker.tone(659, 95);
        delay(125);
        M5.Speaker.tone(784, 130);
        delay(175);
...
static void drawBootScreen(uint8_t progress, const char* status) {
    ...
    drawBootTextCentered("SIGNAL DECK v4.34", 114, 1, TFT_CYAN);
...
static void drawScreenSaver() {
    ...
    drawBootTextCentered("SIGNAL DECK v4.34", 112, 1, TFT_CYAN);
...
static void drawDebugMenuFrame() {
    ...
    int visibleCount = 5;
    if ((int)debugMenuItemCount < visibleCount) {
        visibleCount = (int)debugMenuItemCount;
    }
    int scrollOffset = 0;
    if ((int)debugMenuItemCount > visibleCount && debugMenuIndex >= visibleCount) {
        scrollOffset = debugMenuIndex - (visibleCount - 1);
    }
    int maxOffset = (int)debugMenuItemCount - visibleCount;
    if (scrollOffset > maxOffset) scrollOffset = maxOffset;

    for (int i = 0; i < visibleCount; i++) {
        int itemIndex = scrollOffset + i;
        int y = 66 + (i * 20);
        if (itemIndex == debugMenuIndex) {
            M5.Display.fillRect(28, y - 2, 264, 18, TFT_DARKGREY);
            M5.Display.setTextColor(TFT_WHITE, TFT_DARKGREY);
        } else {
            M5.Display.setTextColor(TFT_WHITE, TFT_BLACK);
        }
        M5.Display.drawString(debugMenuItems[itemIndex], 36, y);
    }

    M5.Display.drawFastHLine(28, 172, 264, TFT_DARKGREY);
    M5.Display.setTextColor(TFT_DARKGREY, TFT_BLACK);
    M5.Display.drawString(debugMenuDescriptions[debugMenuIndex], 30, 184);
}
...
static void playStartupChimeSoft() {
    float previousVolume = audioSystem.getVolumeLevel();
    audioSystem.setVolume(previousVolume * 0.55f);
...
static void drawDebugPanelFrame(const char* title) {
    M5.Display.fillRect(20, 40, 280, 160, TFT_BLACK);
    M5.Display.drawRect(20, 40, 280, 160, TFT_DARKGREY);
    M5.Display.setTextSize(1);
    M5.Display.setTextColor(TFT_CYAN, TFT_BLACK);
    M5.Display.drawString(title, 30, 46);
    M5.Display.setTextColor(TFT_DARKGREY, TFT_BLACK);
    M5.Display.drawString("B: Back", 244, 184);
}

static void drawGpsStatusView() {
    drawDebugPanelFrame("GPS Status");
    int y = 62;
    M5.Display.setTextColor(TFT_WHITE, TFT_BLACK);
    M5.Display.drawString(String("GPS: ") + (gpsEnabled ? "On" : "Off") +
                          "  Port: " + String(gpsPortName()), 30, y); y += 14;
    M5.Display.drawString(String("Fix: ") + (gpsHasFix ? "Yes" : (gpsEverHadFix ? "Stale" : "No")) +
                          "  Sats: " + String(gpsSatellites), 30, y); y += 14;
    M5.Display.drawString(String("HDOP: ") + String(gpsHdop, 1), 30, y); y += 14;
    M5.Display.drawString(String("Lat: ") + (gpsHasFix ? String(gpsLat, 6) : "--"), 30, y); y += 14;
    M5.Display.drawString(String("Lon: ") + (gpsHasFix ? String(gpsLon, 6) : "--"), 30, y); y += 14;
    M5.Display.drawString(String("Course: ") + (gpsHasCourse ? String(gpsCourseDeg, 1) + " deg" : "--"), 30, y); y += 14;
    M5.Display.drawString(String("Geo: ") + geoStatusName() +
                          "  R: " + String(geoRadiusMeters) + "m", 30, y); y += 14;
    M5.Display.drawString(String("Group: ") +
                          (nearestGeoGroupDistanceM >= 0.0f ? String(nearestGeoGroupLabel) + " " + String((int)nearestGeoGroupDistanceM) + "m" : "--"), 30, y); y += 14;
    M5.Display.drawString(String("Near: ") +
                          (nearestGeoDistanceM >= 0.0f ? String((int)nearestGeoDistanceM) + "m " + String(nearestGeoLabel) : "--"), 30, y);
}

static void performGeoFileTest() {
    geoFileTestHasIndex = SD.exists(geoIndexPath);
    geoFileTestGroupRefs = 0;
    geoFileTestGroupsFound = 0;
    geoFileTestActiveGroups = 0;
    geoFileTestMapTargets = 0;
    geoFileTestNearestM = -1.0f;
    strncpy(geoFileTestNearestLabel, "--", sizeof(geoFileTestNearestLabel) - 1);
    geoFileTestNearestLabel[sizeof(geoFileTestNearestLabel) - 1] = '\0';

    File index = SD.open(geoIndexPath, FILE_READ);
    if (index) {
        char line[144];
        while (index.available()) {
            size_t len = index.readBytesUntil('\n', line, sizeof(line) - 1);
            line[len] = '\0';
            char* cr = strchr(line, '\r');
            if (cr) *cr = '\0';
            ...
            geoFileTestGroupRefs++;
            if (SD.exists(path)) {
                geoFileTestGroupsFound++;
            }
        }
        index.close();
    }

    if (gpsHasFix && geoFileTestHasIndex) {
        selectGeoGroupsFromIndex();
        geoFileTestActiveGroups = static_cast<uint16_t>(geoActiveGroupCount);
        clearGeoMapTargets();
        ...
        char bestLabel[24] = "--";
        for (size_t i = 0; i < geoActiveGroupCount; i++) {
            scanGeoTargetFile(geoActiveGroups[i].path, &bestDistance,
                              &bestRadius, bestLabel, sizeof(bestLabel));
        }
        geoFileTestMapTargets = static_cast<uint16_t>(geoMapTargetCount);
        if (bestDistance < 99999999.0f) {
            geoFileTestNearestM = bestDistance;
            strncpy(geoFileTestNearestLabel, bestLabel, sizeof(geoFileTestNearestLabel) - 1);
            geoFileTestNearestLabel[sizeof(geoFileTestNearestLabel) - 1] = '\0';
        }
    }
}

static void drawGeoFileTestView() {
    drawDebugPanelFrame("Geo File Test");
    int y = 62;
    M5.Display.setTextColor(TFT_WHITE, TFT_BLACK);
    M5.Display.drawString(String("Index: ") + (geoFileTestHasIndex ? "Found" : "Missing"), 30, y); y += 16;
    M5.Display.drawString(String("Group refs: ") + String(geoFileTestGroupRefs), 30, y); y += 16;
    M5.Display.drawString(String("Files found: ") + String(geoFileTestGroupsFound), 30, y); y += 16;
    M5.Display.drawString(String("GPS fix: ") + (gpsHasFix ? "Yes" : "No"), 30, y); y += 16;
    M5.Display.drawString(String("Active groups: ") + String(geoFileTestActiveGroups), 30, y); y += 16;
    M5.Display.drawString(String("Map targets: ") + String(geoFileTestMapTargets), 30, y); y += 16;
    M5.Display.drawString(String("Nearest: ") +
                          (geoFileTestNearestM >= 0.0f ? String((int)geoFileTestNearestM) + "m " + String(geoFileTestNearestLabel) : "--"), 30, y);
}

static bool runSdWriteTest() {
    File file = SD.open("/debug_test.txt", FILE_APPEND);
    if (!file) return false;
    uint32_t nowMs = millis();
    file.print(nowMs);
    file.print(',');
    file.print(bootSessionId, HEX);
    file.print(',');
    file.print(loggingEnabled ? "logging_on" : "logging_off");
    file.print(',');
    file.println(gpsHasFix ? "gps_fix" : "gps_no_fix");
    file.close();
    return true;
}

static void drawBleScanTestView() {
    drawDebugPanelFrame("BLE Scan Test");
    int y = 62;
    M5.Display.setTextColor(TFT_WHITE, TFT_BLACK);
    M5.Display.drawString(String("Active: ") + (RadioScannerManager::isBluetoothScanning() ? "Yes" : "No"), 30, y); y += 16;
    M5.Display.drawString(String("Starts: ") + String(bleScanStarts), 30, y); y += 16;
    M5.Display.drawString(String("Ends: ") + String(bleScanEnds), 30, y); y += 16;
    M5.Display.drawString(String("Results: ") + String(bleScanResults), 30, y); y += 16;
    M5.Display.drawString(String("Failures: ") + String(bleScanFailures), 30, y); y += 16;
    M5.Display.drawString(String("Devices seen: ") + String(bleDevicesSeen), 30, y); y += 16;
    M5.Display.drawString(String("Last start base: ") + String(bleScanResultsAtStart), 30, y);
}

static void drawWifiChannelTestView() {
    drawDebugPanelFrame("WiFi Ch Test");
    int y = 62;
    M5.Display.setTextColor(TFT_WHITE, TFT_BLACK);
    M5.Display.drawString(String("Channel: ") + String(RadioScannerManager::getCurrentWifiChannel()), 30, y); y += 16;
    M5.Display.drawString(String("1/6/11 Hop: ") + (RadioScannerManager::isCommonChannelMode() ? "On" : "Off"), 30, y); y += 16;
    M5.Display.drawString(String("WiFi frames: ") + String(wifiFramesSeen), 30, y); y += 16;
    M5.Display.drawString(String("Queue drops: ") + String(wifiQueueDrops), 30, y); y += 16;
    M5.Display.drawString(String("RSSI: ") + String(lastRssi), 30, y); y += 16;
    M5.Display.drawString(String("Last MAC: ") + String(lastMacAddress), 30, y); y += 16;
    M5.Display.drawString(String("Threats: ") + String(threatEventsSeen), 30, y);
}

static void debugLogMarker() {
    if (!loggingEnabled) {
        showInfoPopup("Logging Off");
        return;
    }
    logFieldEvent("manual_marker");
    showInfoPopup("Marker logged");
}
    ...
    if (menuMode == MenuMode::Main) {
        ...
        if (M5.BtnA.wasPressed()) {
            ...
            if (debugMenuIndex == 0) {
                ...
                debugLogMarker();
            } else if (debugMenuIndex == 5) {
                menuMode = MenuMode::GpsStatusView;
                lastDebugViewUpdate = 0;
                drawGpsStatusView();
            } else if (debugMenuIndex == 6) {
                performGeoFileTest();
                menuMode = MenuMode::GeoFileTestView;
                lastDebugViewUpdate = 0;
                drawGeoFileTestView();
            } else if (debugMenuIndex == 7) {
                showInfoPopup(runSdWriteTest() ? "SD Write OK" : "SD Write Fail");
            } else if (debugMenuIndex == 8) {
                menuMode = MenuMode::BleScanTestView;
                lastDebugViewUpdate = 0;
                drawBleScanTestView();
            } else if (debugMenuIndex == 9) {
                menuMode = MenuMode::WifiChannelTestView;
                lastDebugViewUpdate = 0;
                drawWifiChannelTestView();
            } else if (debugMenuIndex == 10) {
                menuMode = MenuMode::Main;
                drawMenuFrame();
            }
        }
    } else if (menuMode == MenuMode::GpsStatusView ||
               menuMode == MenuMode::GeoFileTestView ||
               menuMode == MenuMode::BleScanTestView ||
               menuMode == MenuMode::WifiChannelTestView) {
...
static void handleHomeScreen() {
    ...
    if (menuMode != MenuMode::None) {
        ...
        if (now - lastDebugViewUpdate >= 1000) {
            if (menuMode == MenuMode::GpsStatusView) {
                drawGpsStatusView();
                lastDebugViewUpdate = now;
            } else if (menuMode == MenuMode::BleScanTestView) {
                drawBleScanTestView();
                lastDebugViewUpdate = now;
            } else if (menuMode == MenuMode::WifiChannelTestView) {
                drawWifiChannelTestView();
                lastDebugViewUpdate = now;
            }
        }
